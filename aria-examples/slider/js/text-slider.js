...
window.addEventListener('load', function () {
  ...
  [].forEach.call(sliders, function(slider) {
    if (slider && !slider.done) {
      var s = new aria.widget.tslider(slider);
      s.initSlider();
    }  
  });
...
aria.widget.tslider = function(node, options, width) {
  ...
  this.valueMin = 0;
  this.valueMax = options.length;

  this.valueNow = 0;
  this.valueText = parseInt(this.thumb.getAttribute('aria-valuetext'));
  
  if (!this.valueNow  || (this.valueText === '')) {
    this.valueNow = 0;
    this.valueText = options[0];
  }
...
aria.widget.tslider.prototype.updateThumbPosition = function() {
  ...
  aria.widget.tslider.updateColorBox();
...
aria.widget.tslider.updateColorBox = function() {
